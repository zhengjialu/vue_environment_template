          ...
          <template v-for="oneChildrenItem in item.children">
            <template v-if="oneChildrenItem.title && !oneChildrenItem.children">
              <el-menu-item :key="oneChildrenItem.key" :index="oneChildrenItem.key">
                <router-link :to="oneChildrenItem.path">{{ oneChildrenItem.title }}</router-link>
              </el-menu-item>
            </template>
            <template v-else-if="oneChildrenItem.children">
              <el-submenu :key="oneChildrenItem.key" :index="oneChildrenItem.key">
              <template slot="title">{{ oneChildrenItem.title }}</template>
                <!-- 第三级菜单 -->
                <el-menu-item v-for="twoChildrenItem in oneChildrenItem.children" :key="twoChildrenItem.key" :index="twoChildrenItem.key">
                  <router-link :to="twoChildrenItem.path">{{ twoChildrenItem.title }}</router-link>
                </el-menu-item>
              </el-submenu>
            </template>
          </template>
